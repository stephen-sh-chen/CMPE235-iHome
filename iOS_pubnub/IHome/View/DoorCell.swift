...
class DoorCell: UITableViewCell, SFSpeechRecognizerDelegate, PNObjectEventListener  {
    ...
    func client(_ client: PubNub, didReceiveMessage message: PNMessageResult) {
        ...
        if (_types == "Door" && _name as String == name.text!) {
            self.status = _action as String!;
            if status == "ON" {
                DispatchQueue.main.async() {
                    self.onAndOff.setBackgroundImage(UIImage(named : "icons8-door-opened-40"), for: UIControlState.normal)
                }
            } else {
                DispatchQueue.main.async() {
                    self.onAndOff.setBackgroundImage(UIImage(named: "icons8-door-closed-40"), for: UIControlState.normal)
                }
            }
        }
        print(self.status)
    ...
    @IBAction func OnOff(_ sender: Any) {
        if status == "ON"{
            //onAndOff.setBackgroundImage(UIImage(named : "icons8-door-opened-40"), for: UIControlState.normal)
            //status = "OFF"
            //<++++++++ Stephen
            let originalObjects = [myPubNubMessage(Name: name.text!, Types: "Door", Action: "OFF")]
            ...
            //onAndOff.setBackgroundImage(UIImage(named: "icons8-door-closed-40"), for: UIControlState.normal)
            //status = "ON"
            //<++++++++ Stephen
            let originalObjects = [myPubNubMessage(Name: name.text!, Types: "Door", Action: "ON")]
            let encoder = JSONEncoder()
            let data = try! encoder.encode(originalObjects)
            print(data)
            print(String(data: data, encoding: .utf8)!)
