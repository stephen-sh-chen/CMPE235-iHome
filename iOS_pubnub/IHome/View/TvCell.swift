...
class TvCell: UITableViewCell, SFSpeechRecognizerDelegate, PNObjectEventListener  {
    ...
    func client(_ client: PubNub, didReceiveMessage message: PNMessageResult) {
        ...
        if (_types == "TV" && _name as String == name.text!) {
            self.status = _action as String!;
            if status == "ON" {
                DispatchQueue.main.async() {
                    self.onAndOff.setBackgroundImage(UIImage(named : "icons8-tv-on-40"), for: UIControlState.normal)
                }
            } else {
                DispatchQueue.main.async() {
                    self.onAndOff.setBackgroundImage(UIImage(named: "icons8-tv-off-40"), for: UIControlState.normal)
                }
            }
        }
        print(self.status)
    ...
    @IBAction func OnOff(_ sender: Any) {
        if status == "ON"{
            //<++++++++ Stephen
            let originalObjects = [myPubNubMessage(Name: name.text!, Types: "TV", Action: "PAUSE")]
            ...
            //<++++++++ Stephen
            let originalObjects = [myPubNubMessage(Name: name.text!, Types: "TV", Action: "ON")]
            let encoder = JSONEncoder()
            let data = try! encoder.encode(originalObjects)
            print(data)
            print(String(data: data, encoding: .utf8)!)
    ...
    func sendRequestToPubNub(commandToBeSent:String) {
        ...
        } else if (getCommand == "off" || getCommand == "close" || getCommand == "pause") {
            let originalObjects = [myPubNubMessage(Name: name.text!, Types: "TV", Action: "PAUSE")]
            let encoder = JSONEncoder()
            let data = try! encoder.encode(originalObjects)
            print(data)
    ...
    func configureCell(lableName : String, newStatus : String){
        ...
        if status == "ON"{
            onAndOff.setBackgroundImage(UIImage(named : "icons8-tv-on-40"), for: UIControlState.normal)
            //status = "OFF"
            
        }
        else{
            onAndOff.setBackgroundImage(UIImage(named: "icons8-tv-off-40"), for: UIControlState.normal)
            //status = "ON"
        }
    }
